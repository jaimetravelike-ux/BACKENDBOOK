  ...
  input_schema: {
    ...
    properties: {
      ...
      noSpecificHotel: {
        type: 'boolean',
        description:
          'true si el cliente NO ha nombrado ningun hotel concreto por su nombre y lo que quiere es que le enseñes varias opciones de la zona. Esto incluye tanto "no quiero ninguno de esos, busca en la zona" (tras una desambiguacion) COMO la peticion inicial, desde el primer mensaje, cuando pide hoteles de una zona/barrio en general sin dar un nombre de hotel (p.ej. "los mejores hoteles cerca de Times Square", "algo con buena relacion calidad-precio en Chelsea", "cualquier hotel en Brooklyn", "hoteles cerca de X"). IMPORTANTE: esto tambien aplica en singular - "el hotel mas caro/barato/lujoso de Nueva York" NO es el nombre de un hotel concreto, es pedir que elijas tu el mejor segun ese criterio (igual que "los hoteles mas caros/baratos/lujosos"): pon noSpecificHotel:true tambien en ese caso. Ponlo a false (o no lo toques) SOLO cuando el cliente si menciona el nombre concreto y real de un hotel.',
      },
      pricePreference: {
        type: 'string',
...
- MUY IMPORTANTE - distingue desde el primer mensaje entre pedir un hotel concreto y pedir una zona: si el cliente NO da el nombre de ningun hotel y solo pide una zona/barrio en general (p.ej. "los mejores hoteles cerca de Times Square", "algo con buena relacion calidad-precio en Chelsea", "cualquier hotel en Brooklyn", "hoteles cerca de X"), llama a update_booking_slots con noSpecificHotel:true YA EN ESE MISMO TURNO, no esperes a que rechace una lista de hoteles primero - si esperas, la busqueda acaba resolviendo a un unico hotel al azar en vez de mostrarle varias opciones reales de la zona, que es justo lo que pidio. Esto aplica igual en singular: "el hotel mas caro/barato/lujoso de Nueva York" tambien es noSpecificHotel:true (no es el nombre de un hotel, es pedir que tu elijas el mejor segun ese criterio) - no lo confundas con una peticion de un hotel concreto solo porque diga "el hotel" en singular.
